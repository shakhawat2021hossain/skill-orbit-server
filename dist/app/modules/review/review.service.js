...
const postReview = async (payload, userId) => {
    ...
    if (!existingReview) {
        // create new review
        const newReview = await Review.create({ studentId: userId, courseId, rating, review: review || "", reviewer: user?.name });
        // update course rating
        const oldCount = course.rating?.count ?? 0;
        const oldAvg = course.rating?.average ?? 0;
        const newCount = oldCount + 1;
        const newAvg = ((oldAvg * oldCount) + rating) / newCount;
        course.rating = { count: newCount, average: Number(newAvg.toFixed(2)) };
        await course.save();
        return newReview;
    }
    else {
        ...
        course.rating = { count, average: Number(newAvg.toFixed(2)) };
        await course.save();

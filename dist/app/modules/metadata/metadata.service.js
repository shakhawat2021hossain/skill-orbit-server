...
const getAdminMetaData = async () => {
    const [totalUsers, blockedUsers, totalCourses, publishedCourses, totalEnrollments, paidEnrollments] = await Promise.all([
        User.countDocuments(),
    ...
    const totalRevenue = paidEnrollments.reduce((sum, enroll) => sum + (enroll.amountPaid || 0), 0);
    return {
        users: {
            total: totalUsers,
...
const getInstructorMetaData = async (instructorId) => {
    ...
    const totalIncome = paidEnrollments.reduce((sum, enroll) => sum + (enroll.amountPaid || 0), 0);
    return {
        courses: {
...
const getStudentMetaData = async (studentId) => {
    const enrollments = await Enrollment.find({
        studentId,
        paymentStatus: PaymentStatus.PAID,
    });
    const completedCourses = enrollments.filter(enroll => enroll.progress === 100).length;
    const totalSpent = enrollments.reduce((sum, enroll) => sum + (enroll.amountPaid || 0), 0);
